...
No existing hosted merchant route compares two caller-supplied pricing-row
JSON objects. `GET /extract` fetches a URL. `POST /lockfile-pin-delta`
compares npm lockfiles. Neither is vendor-budget impact.
...
Not built: a second payment rail, job platform, SDS kit, queue, or ledger.
Caller inputs cannot select programs, paths, URLs, or writable caches.
Temporary artifacts are in-process worker stdio only; workers are reaped.
