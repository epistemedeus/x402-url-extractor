...
app.get("/healthz", async (_req, res) => {
  ...
  res.json({
    ...
    prices: {
      ...
      read: READ_PRICE,
      scan: SCAN_PRICE,
      schemaforge: SCHEMAFORGE_PRICE,
      enrich: ENRICH_PRICE,
      "wallet-enrich": WALLET_ENRICH_PRICE,
      "deep-audit": DEEP_AUDIT_PRICE,
      "morpho-position": MORPHO_POSITION_PRICE,
      "morpho-protection": MORPHO_PROTECTION_PRICE,
      "morpho-market-underwrite": MORPHO_MARKET_UNDERWRITE_PRICE,
      "morpho-preliquidation-replay": MORPHO_PRELIQUIDATION_REPLAY_PRICE,
      "opportunity-preflight": OPPORTUNITY_PREFLIGHT_PRICE,
      "agent-discoverability-audit": AGENT_DISCOVERABILITY_AUDIT_PRICE,
      "payment-offer-preflight": PAYMENT_OFFER_PREFLIGHT_PRICE,
      "seller-integrity-audit": SELLER_INTEGRITY_AUDIT_PRICE,
      "contract-qualified-search": CONTRACT_QUALIFIED_SEARCH_PRICE,
      "agent-surface-budget-audit": AGENT_SURFACE_BUDGET_AUDIT_PRICE,
      "settlement-proof": SETTLEMENT_PROOF_PRICE,
      "transaction-receipt": TRANSACTION_RECEIPT_PRICE,
...
const RESOURCES = [
  ...
  { url: `${PUBLIC_URL}/distribution/agent-discoverability-audit`, amount: priceToAtomic(AGENT_DISCOVERABILITY_AUDIT_PRICE), description: "Audit one x402 or MPP service across ten public machine-discovery views. Returns intent rank, canonical and alias records, duplicates, competitors, expected-route presence, exact-price drift, evidence-based next actions, and source outages. Optional checks compare a same-origin unsigned offer, public Agent Card, ERC-8004 document, action catalog, and Coinbase validation-versus-materialization state. Uses no catalog credential, signature, or payment.", mimeType: "application/json" },
  { url: `${PUBLIC_URL}/commerce/payment-offer-preflight`, amount: priceToAtomic(PAYMENT_OFFER_PREFLIGHT_PRICE), description: "Compare x402 and MPP challenges plus seller-declared response readiness before buyer authorization for one exact public HTTPS GET URL. Optionally checks catalog metadata against live unsigned offers across request, protocol, amount, network, asset, recipient, and expiry. Returns normalized offers, parity findings, response-contract evidence, and an explicit decision. Uses no credential, signature, or target payment and reads no paid body.", mimeType: "application/json" },
...
const buildOpenApiDocument = ({ profile = "agentcash" } = {}) => {
  const document = {
    ...
    paths: {
      ...
      "/commerce/seller-integrity-audit": {
        get: {
          ...
          parameters: [
            ...
            { name: "referral", in: "query", required: false, description: "Optional privacy-bounded referral ID derived from a complete seller-signed x402 offer-receipt envelope. It labels acquisition only and cannot change price, authorization, or delivery.", schema: { type: "string", pattern: "^r1_[0-9a-f]{64}$" } },
          ],
          responses: {
      ...
      "/wallet-enrich": { get: { summary: RESOURCES[5].description, parameters: [{ name: "address", in: "query", required: true, schema: { type: "string" } }], responses: { "200": { description: "agent-ready on-chain profile (type, native + token holdings, contract/token metadata, proxy, activity, profile label)" }, "402": { description: `payment required (x402, ${WALLET_ENRICH_PRICE} USDC base)` }, "502": { description: "Base RPC unavailable; no wallet type was inferred", content: { "application/json": { schema: { type: "object", properties: { ok: { type: "boolean", const: false }, address: { type: "string" }, error: { type: "string", const: "wallet_enrichment_upstream_unavailable" } }, required: ["ok", "address", "error"], additionalProperties: false } } } } } } },
          ...
          ...declareDiscoveryContract({
            ...
            inputSchema: {
              ...
              properties: {
                ...
                referral: { type: "string", pattern: "^r1_[0-9a-f]{64}$", description: "Optional receipt-derived acquisition label. It does not affect payment or delivery." },
              },
...
app.get("/wallet-enrich", async (req, res) => {
  ...
  try {
    ...
    res.status(502).json({ ok: false, address, error: "wallet_enrichment_upstream_unavailable" });
  }
});
