...
const RESOURCES = [
  ...
  { url: `${PUBLIC_URL}/distribution/agent-discoverability-audit`, amount: priceToAtomic(AGENT_DISCOVERABILITY_AUDIT_PRICE), description: "Buyer-intent rank audit for one x402 or MPP service across Coinbase Bazaar, Coinbase Agentic Market, Agent402, Circle Agent Marketplace, AgenticTrade, and the official MPP catalog. Returns registry-native position, source-family-aware coverage, competitors above the target, expected-route presence, coverage gaps, evidence-based next actions, source outages, and explicit method limits. No catalog credentials or payments.", mimeType: "application/json" },
...
import("./mcp-server.mjs")
  .then(({ mountMcp }) =>
    mountMcp(app, {
      ...
      network: NETWORK,
      payTo: PAY_TO,
      serverInfo: { name: "x402-data-gateway", version: "1.11.32" },
      tools: [
