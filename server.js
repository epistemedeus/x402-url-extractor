...
const RESOURCES = [
  ...
  { url: `${PUBLIC_URL}/commerce/seller-integrity-audit`, amount: priceToAtomic(SELLER_INTEGRITY_AUDIT_PRICE), description: "Diagnose one exact paid GET or POST route after a buyer integration fails, a seller changes the route, or before the next paid retry or release. Checks constructible non-secret input, live unpaid GET terms, optional Bazaar metadata, and buyer-required success paths. Returns machine_buyable, contract_ready, or repair_required with exact repair actions and an optional receipt-derived referral experiment. Uses no credential or target payment, follows no redirect, and sends no target POST.", mimeType: "application/json" },
