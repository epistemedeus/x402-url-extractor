...
const RESOURCES = [
  ...
  { url: `${PUBLIC_URL}/distribution/agent-discoverability-audit`, amount: priceToAtomic(AGENT_DISCOVERABILITY_AUDIT_PRICE), description: "Buyer-intent rank and catalog-to-runtime price-coherence audit for one x402 or MPP service across ten public machine-service discovery views. Returns registry-native position, dependency-labeled coverage, competitors, expected-route presence, exact-price drift, evidence-based next actions, and source outages. Optional runtimeUrl derives the reference price from a same-origin unsigned headers-only offer; optional surfaceAudit checks the public Agent Card, ERC-8004 registration document, and action catalog. No catalog credentials, signatures, or payments.", mimeType: "application/json" },
          ...
          ...declareDiscoveryContract({
            ...
            output: {
              example: {
                ...
                summary: { sourceCount: 10, availableSourceCount: 10, targetFoundSourceCount: 4, expectedRouteFoundSourceCount: 4, sourceFamilyCount: 9, targetFoundSourceFamilyCount: 4, priceReference: { basis: "live_unsigned_offer", amountAtomic: "5000", amountUsd: 0.005, protocols: ["mpp", "x402"] }, matchedPriceSourceCount: 3, driftedPriceSourceCount: 1 },
...
app.get("/", (req, res) => {
  const gateway = {
    ...
    paidRoutes: {
      ...
      "GET /distribution/agent-discoverability-audit?origin=&intent=&route=&runtimeUrl=&payTo=&surfaceAudit=": `${AGENT_DISCOVERABILITY_AUDIT_PRICE} - brand-blind agent discovery rank, dependency-labeled coverage, expected-route presence, runtime-derived catalog-price coherence, optional seller-owned surface coverage, and competing results across ten machine-service views.`,
