...
test("valid MPP credential survives an unparseable x402-family header", async () => {
  const challenge = Challenge.Schema.parse({
    id: "ch_0123456789abcdef",
    realm: "agents.samedaydesk.com",
    method: "evm",
    intent: "charge",
    opaque: "Y2hhbGxlbmdlLW9wYXF1ZQ",
    request: {
  ...
  const dataDir = await mkdtemp(path.join(os.tmpdir(), "commerce-dual-header-"));
  try {
    const telemetry = createCommerceTelemetry({ dataDir, secret: "dual-header-secret" });
    const listeners = new Map();
    const req = {
      path: "/extract",
      url: "/extract",
      method: "GET",
      headers: { authorization, "payment-signature": "junk" },
      query: {},
      ip: "203.0.113.80",
      socket: {},
    };
    ...
    await telemetry.flush();
    const snapshot = await telemetry.snapshot({ days: 1 });
    assert.equal(snapshot.paymentHeaderEvents, 1);
    assert.equal(snapshot.parseableCredentialAttemptEvents, 1);
    assert.equal(snapshot.unparseablePaymentHeaderEvents, 0);
    assert.equal(snapshot.credentialAttemptByProtocol.mpp, 1);
  } finally {
    await rm(dataDir, { recursive: true, force: true });
  }
});
