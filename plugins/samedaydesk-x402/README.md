...
- Not an MCP Registry publish. The live remote is already
  `io.github.epistemedeus/x402-data-gateway@1.23.16`.
...
Clone the isolated branch, then point a host at this directory. Default
`master` does not contain this package.

```bash
git clone --branch grok/agent-plugins-samedaydesk-x402-20260820 \
  https://github.com/epistemedeus/x402-url-extractor.git
...
After merge, Copilot also documents a subdirectory install of the form
`owner/repo:plugins/samedaydesk-x402`. That form reads the default branch
