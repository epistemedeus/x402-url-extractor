...
test("mapping coverage is 4 covered / 1 partial / 2 gaps from the mapping file", () => {
  ...
  assert.equal(layer2.mapping.confirmation, "author+replay-confirmed");
  assert.equal(layer2.mapping.confirmationScope, "pinned revision only");
...
test("composition records 4/1/2 as author+replay-confirmed at the pinned revision only", () => {
  ...
  assert.equal(result.mappingCoverage.confirmationScope, "pinned revision only");
...
test("a mapping that claims full coverage is not confirmed", () => {
  ...
  assert.throws(
    () => loadLayer2({ mappingPath: path }),
    (error) => error instanceof BasePayCompositionError && /git blob|mapping/.test(error.message),
  );
});
