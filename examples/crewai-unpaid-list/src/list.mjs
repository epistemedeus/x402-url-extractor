import { CREWAI_TRANSPORT, LIVE_MCP_URL, MCP_PROTOCOL } from "./constants.mjs";
import { fail } from "./errors.mjs";
import { acceptInventory } from "./accept.mjs";
import { unpaidBoundary } from "./boundary.mjs";
import { loadCrewaiConfig, mcpServerAdapterParams } from "./crewai-config.mjs";
...
export async function unpaidToolsList({
  ...
  const config = loadCrewaiConfig(configPath);
  const adapter = mcpServerAdapterParams(config);
