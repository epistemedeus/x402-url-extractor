import { CLIENT_VERSION, REPORT_SCHEMA } from "./constants.mjs";
...
export function printHuman(report) {
  ...
  const lines = [`samedaydesk crewai unpaid list  ${status}  ${report.command}`];
  ...
  if (report.boundary) {
    lines.push(`  paymentSent  ${report.boundary.paymentSent}  toolsCalled  ${report.boundary.toolsCalled}`);
  }
