...
test("CLI inspect then --approve against mounted merchant; refuse, replay, concurrent", { timeout: 180_000, skip }, async (t) => {
  ...
  // Recover the exact credential generated by the maintained CLI after a real
  // merchant process restart. This path must neither sign nor settle again.
  const uncertainSends = (await readFile(payLog, "utf8")).trim().split("\n").filter(Boolean).map(line => JSON.parse(line));
  const uncertainSend = uncertainSends.at(-1);
  const verifiesBeforeRestart = facilitator.calls.verify;
  await stopChild(merchant.child);
  merchant = await startMerchant({ dataDir, facilitatorUrl: facilitator.url });
  const recovery = await proxyToMerchant(merchant.base)(LIVE_VENDOR_BUDGET_URL, {
    method: "POST",
    headers: { "content-type": "application/json", "payment-signature": uncertainSend.header },
    body: uncertainSend.body,
  });
  ...
  assert.deepEqual(recovered.delivery, uncertainJson.evidence.retainedBody.delivery);
  assert.equal(facilitator.calls.verify, verifiesBeforeRestart);
  assert.equal(facilitator.calls.settle, 5);
});
