...
function observedReject(code, kind, message, details) {
  return { ok: false, code, kind, message, details: details || null };
}
...
export function evaluateRecorded(seed) {
  ...
  if (Array.isArray(methods) && methods.includes("tools/call")) {
    return observedReject(
      "BOUNDARY_REFUSED",
      "tools_called",
      "recorded transcript includes tools/call",
      { methods },
    );
  }
  if (Array.isArray(methods)) {
    for (const method of methods) {
      if (/publish|registry|marketplace/i.test(String(method || ""))) {
        return observedReject(
          "BOUNDARY_REFUSED",
          "publish",
          `recorded transcript includes publish method ${method}`,
          { methods },
        );
      }
    }
  ...
  for (const name of Object.keys(headers)) {
    if (headerLooksPaidOrCredential(name)) {
      return observedReject(
        "BOUNDARY_REFUSED",
        "payment_header",
        `recorded request has refused header ${name}`,
        { header: name },
      );
    }
  }
  const recordedUrl = seed.recorded?.url || seed.mcp?.url;
  if (hostLooksNeo(recordedUrl)) {
    return observedReject(
      "BOUNDARY_REFUSED",
      "neo_host",
      "recorded request targets a refused neo host",
      { url: recordedUrl },
    );
  }
