/**
 * Tiny c43 helpers: PIN load, fixture hashes, offline CLI spawn.
 * No network. Does not implement freshness-receipt.
 */
...
export function loadCase(pin, caseId) {
  const row = (pin.primaryFixtures || []).find((item) => item.role === "case" && item.id === caseId);
  if (!row) throw new Error(`PIN missing case ${caseId}`);
  return { ...row, doc: loadJson(repoFile(row.path)) };
}
