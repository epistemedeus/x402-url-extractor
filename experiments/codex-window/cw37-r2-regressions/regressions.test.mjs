...
test('F7 live operation metadata, MCP construction and signed route coverage', { timeout: 90000 }, async t => {
  ...
  try {
    ...
    try {
      ...
      await check(t, 'F6.mcp-raw-rounding-rejected', async row => {
        const count = m.eventsRead().filter(x => x.path === '/settle').length;
        const response = await http('/mcp', {raw: '{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"vendor_budget_impact","arguments":' + pair('0.1', '0.10000000000000001') + '}}', headers: {accept: 'application/json, text/event-stream'}});
        row.witness = {status: response.status, settlementCallsBefore: count, settlementCallsAfter: m.eventsRead().filter(x => x.path === '/settle').length};
        assert.ok([400,403,422].includes(response.status));
        assert.equal(row.witness.settlementCallsAfter, count);
      });
    ...
    await check(t, 'F7.deployment-attestation-exact-route', async row => {
      ...
      assert.notEqual(vendor.decision, 'verified_exact_binding');
      const catalog = (await http('/api/actions')).body;
      const coverage = catalog.actions.find(x => x.route === ROUTE).deploymentAttestation;
      row.witness.advertisedCoverage = coverage;
      assert.equal(coverage.covered, false);
      assert.equal(coverage.status, 'uncovered');
      const manifest = (await http('/.well-known/agent-payment-evidence.json')).body;
      assert.equal(manifest.evidence.operationDeploymentCoverage.find(x => x.path === ROUTE).covered, false);
    });
  } finally { await m.stop(); }
});

test('F7 production vendor enablement requires existing exact signed coverage', async t => {
  await check(t, 'F7.production-coverage-gate', async row => {
    const m = await merchant({env: {NODE_ENV: 'production'}});
    try {
      row.witness = {ready: m.ready, logs: m.logs()};
      assert.equal(m.ready, false);
      assert.match(m.logs(), /service deployment does not attest POST \/vendor-budget-impact/);
    } finally { await m.stop(); }
  });
});
