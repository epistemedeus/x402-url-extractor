...
function help() {
  return `w811: x402 undeclared path is route_absent, not HTTP 402 or settlement.

Usage:
  node tests/protocol/w811-route-absent/check.mjs
  node tests/protocol/w811-route-absent/check.mjs --cold
  node tests/protocol/w811-route-absent/check.mjs --seeded-failure
  node tests/protocol/w811-route-absent/check.mjs --all-fixtures
  node tests/protocol/w811-route-absent/check.mjs <fixture.json>
...
async function main(argv = process.argv.slice(2)) {
  ...
  if (argv[0] === "--all-fixtures") {
    ...
    process.stderr.write(
      `w811-route-absent fixtures: ${report.ok ? "pass" : "fail"} counted=${report.counted} passed=${report.passed}\n`,
    );
  ...
  if (argv[0] === "--seeded-failure") {
    ...
    print({
      ...report,
      fixture: "tests/protocol/w811-route-absent/fixtures/reject/seeded-absent-route-as-402.json",
    });
    if (report.ok) {
  ...
  if (argv.length === 0 || argv[0] === "--cold") {
    ...
    print(report);
    process.stderr.write(
      `w811-route-absent cold: ${report.ok ? "pass" : "fail"} code=${report.code}`
  ...
  if (fixture.expect === "reject") {
    if (report.ok) {
      process.stderr.write("reject fixture must not pass w811-route-absent\n");
      return 2;
    }
