...
/**
 * Evaluate one w831 route_absent trace.
 *
...
export function evaluateTrace(trace = {}) {
  ...
  if (boundary.paymentSent === true) {
    violations.push(violation(CODES.PAYMENT_SENT, "trace sent a live payment; w831 must not pay"));
  }
  ...
  if (boundary.checkoutMutated === true) {
    violations.push(violation(CODES.CHECKOUT_MUTATED, "w831 must not mutate checkout"));
  }
  if (boundary.published === true) {
    violations.push(violation(CODES.PUBLISHED, "w831 must not publish"));
  }
  if (boundary.neoTouched === true) {
    violations.push(violation(CODES.NEO_TOUCHED, "w831 must not touch neomorphic-io"));
  }
