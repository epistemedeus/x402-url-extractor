...
export function createEmptyCatalogFetch() {
  return async (url) => {
    ...
    if (body) return body;
    throw new Error(`w911-route-absent blocked unexpected catalog URL: ${target}`);
  };
}
...
async function refusePay() {
  throw new Error("w911-route-absent must not preflight, verify, or settle");
}
...
async function startFakeFacilitator() {
  ...
  const server = createHttpServer((req, res) => {
    ...
    if (req.method === "POST" && req.url === "/verify") {
      calls.verify += 1;
      throw new Error("w911-route-absent must not verify");
    }
    if (req.method === "POST" && req.url === "/settle") {
      calls.settle += 1;
      throw new Error("w911-route-absent must not settle");
    }
...
export async function withMerchantFlagOff(fn) {
  const dataDir = await mkdtemp(join(tmpdir(), "w911-route-absent-"));
