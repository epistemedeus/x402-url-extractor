...
function help() {
  return `w911: x402 route_absent is a checked miss, not demand.

Usage:
  node tests/protocol/w911-route-absent/check.mjs
  node tests/protocol/w911-route-absent/check.mjs --cold
  node tests/protocol/w911-route-absent/check.mjs --seeded-failure
  node tests/protocol/w911-route-absent/check.mjs --all-fixtures
  node tests/protocol/w911-route-absent/check.mjs <fixture.json>
...
async function main(argv = process.argv.slice(2)) {
  ...
  if (argv[0] === "--all-fixtures") {
    ...
    process.stderr.write(
      `w911-route-absent fixtures: ${report.ok ? "pass" : "fail"} counted=${report.counted} passed=${report.passed}\n`,
    );
  ...
  if (argv[0] === "--seeded-failure") {
    ...
    print({
      ...report,
      fixture: "tests/protocol/w911-route-absent/fixtures/reject/seeded-route-absent-as-demand.json",
    });
    if (report.ok) {
  ...
  if (argv.length === 0 || argv[0] === "--cold") {
    ...
    print(report);
    process.stderr.write(
      `w911-route-absent cold: ${report.ok ? "pass" : "fail"} code=${report.code}`
  ...
  if (fixture.expect === "reject") {
    if (report.ok) {
      process.stderr.write("reject fixture must not pass w911-route-absent\n");
      return 2;
    }
