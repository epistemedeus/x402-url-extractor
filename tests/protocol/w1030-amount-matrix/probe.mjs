...
import { MATRIX, PAYMENT_REQUEST_HEADER_NAMES, SCHEMA_FIXTURE, SDS } from "./constants.mjs";
  ...
  const initialize = await fetch(`${base}/mcp`, {
    ...
    body: JSON.stringify({
      ...
      params: {
        ...
        capabilities: {},
        clientInfo: { name: "w1030-amount-matrix", version: "0.0.1" },
      },
    }),
  });
...
export async function startFakeFacilitator() {
  ...
  const server = createHttpServer((req, res) => {
    ...
    if (req.method === "GET" && (req.url === "/supported" || req.url?.startsWith("/supported?"))) {
      calls.supported += 1;
      return send(200, {
    ...
    if (req.method === "POST" && req.url === "/verify") {
      calls.verify += 1;
      return send(400, { isValid: false, invalidReason: "w1030_unpaid_matrix_refuses_verify" });
    }
    if (req.method === "POST" && req.url === "/settle") {
      calls.settle += 1;
      return send(400, { success: false, errorReason: "w1030_unpaid_matrix_refuses_settle" });
    }
    return send(404, { error: "unexpected_w1030_facilitator_request" });
  });
...
export async function startLocalMerchant() {
  ...
  const dataDir = await mkdtemp(join(tmpdir(), "w1030-amount-matrix-"));
