...
function openapiGet(openapi, path) {
  return asRecord(asRecord(openapi?.paths)?.[path])?.get || null;
}
...
export function evaluateAmountMatrix(document) {
  ...
  for (const route of MATRIX) {
    ...
    const openapiObs = openapiGet(openapi, route.path);
