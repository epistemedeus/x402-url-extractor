...
function kindCode(kind) {
  if (kind === "payment_required_header") return CODES.PAYMENT_REQUIRED_HEADER;
  if (kind === "jsonrpc_error_not_iserror") return CODES.JSONRPC_ERROR_NOT_ISERROR;
  if (kind === "http_402") return CODES.HTTP_402_NOT_MCP_ISERROR;
  if (kind === "snake_case_is_error") return CODES.SNAKE_CASE_IS_ERROR;
  if (kind === "iserror_omitted") return CODES.ISERROR_OMITTED;
  if (kind === "paid_delivery") return CODES.HTTP_200_CLASSIFIED_AS_CHARGED;
...
export function evaluateToolsList(fixture = {}) {
  ...
  return {
    ...
    code: violations.length === 0 ? CODES.UNPAID_MCP_CHALLENGE : primary.code,
    id: fixture.id ?? null,
    ...
    classified,
    violations,
    claimsRejected: claimsDemandCharge(claims) && violations.some((item) => item.code === CODES.HTTP_200_CLASSIFIED_AS_CHARGED),
  };
}
...
export function evaluateToolsCall(fixture = {}) {
  ...
  if (classified.kind === "jsonrpc_error_not_iserror") {
    violations.push(violation(
      ...
      `JSON-RPC error ${classified.jsonRpcErrorCode} is not MCP result.isError=true`,
    ));
  }
  if (classified.kind === "http_402" || classified.httpStatus === 402) {
    violations.push(violation(
      CODES.HTTP_402_NOT_MCP_ISERROR,
      "MCP tools/call unpaid challenge is HTTP 200 isError, not HTTP 402",
    ));
  }
  if (classified.kind === "snake_case_is_error") {
    violations.push(violation(
      CODES.SNAKE_CASE_IS_ERROR,
      "MCP CallToolResult uses camelCase isError, not is_error",
    ));
  }
  if (classified.kind === "iserror_omitted") {
    violations.push(violation(
      CODES.ISERROR_OMITTED,
      "PaymentRequired on tools/call must set result.isError=true",
    ));
  }
  ...
  if (classified.kind !== "unpaid_mcp_challenge") {
    if (!violations.some((item) => item.code === kindCode(classified.kind)
      ...
      || item.code === CODES.HTTP_200_CLASSIFIED_AS_CHARGED)) {
      violations.push(violation(
        kindCode(classified.kind),
        `tools/call is ${classified.kind}, not unpaid_mcp_challenge`,
      ));
    }
  } else {
    if (classified.httpStatus !== 200) {
      violations.push(violation(CODES.HTTP_402_NOT_MCP_ISERROR, `expected HTTP 200, got ${classified.httpStatus}`));
    }
    ...
    if (classified.hasJsonRpcError) {
      violations.push(violation(
        CODES.JSONRPC_ERROR_NOT_ISERROR,
        "unpaid tools/call must be a JSON-RPC result, not error",
      ));
    }
    ...
    if (classified.hasStructuredContent !== true) {
      violations.push(violation(
        CODES.MALFORMED_FIXTURE,
        "unpaid isError result must include structuredContent PaymentRequired",
      ));
    }
    if (classified.hasTextContent !== true) {
      violations.push(violation(
        CODES.MALFORMED_FIXTURE,
        "unpaid isError result must JSON-encode PaymentRequired in content[0].text",
      ));
    }
  ...
  return {
    ...
    claimsRejected: claimsDemandCharge(claims)
      && violations.some((item) => item.code === CODES.HTTP_200_CLASSIFIED_AS_CHARGED),
  };
}
...
export function evaluateColdSuite({ toolsList, toolsCall, counters, origin, serverInfo }) {
  ...
  return {
    ...
    schema: "samedaydesk.x402-protocol-report.w929-mcp-iserror.v1",
    code: ok ? CODES.UNPAID_MCP_CHALLENGE : (call.ok ? list.code : call.code),
    origin,
    serverInfo,
    paymentSent: false,
