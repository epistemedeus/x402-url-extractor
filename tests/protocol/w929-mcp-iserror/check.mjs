...
function help() {
  return `w929 x402 MCP unpaid tools/call is HTTP 200 isError, not settlement.

Usage:
  node tests/protocol/w929-mcp-iserror/check.mjs
  node tests/protocol/w929-mcp-iserror/check.mjs --cold
  node tests/protocol/w929-mcp-iserror/check.mjs --seeded-failure
  node tests/protocol/w929-mcp-iserror/check.mjs --all-fixtures
  node tests/protocol/w929-mcp-iserror/check.mjs <fixture.json>
...
async function main(argv = process.argv.slice(2)) {
  ...
  if (argv[0] === "--all-fixtures") {
    ...
    process.stderr.write(
      `w929-mcp-iserror fixtures: ${report.ok ? "pass" : "fail"} counted=${report.counted} passed=${report.passed}\n`,
    );
  ...
  if (argv[0] === "--seeded-failure") {
    ...
    print({
      ...report,
      fixture: "tests/protocol/w929-mcp-iserror/fixtures/reject/seeded-http-200-as-charged.json",
    });
    if (report.ok) {
  ...
  if (argv.length === 0 || argv[0] === "--cold") {
    ...
    print(report);
    process.stderr.write(
      `w929-mcp-iserror cold: ${report.ok ? "pass" : "fail"} code=${report.code}`
  ...
  if (fixture.expect === "reject") {
    if (report.ok) {
      process.stderr.write("reject fixture must not pass w929-mcp-iserror\n");
      return 2;
    }
