...
async function startFakeFacilitator() {
  ...
  const server = createHttpServer((req, res) => {
    ...
    if (req.method === "POST" && req.url === "/verify") {
      calls.verify += 1;
      return send(500, { error: "w829 unpaid hop must not verify" });
    }
    if (req.method === "POST" && req.url === "/settle") {
      calls.settle += 1;
      return send(500, { error: "w829 unpaid hop must not settle" });
    }
...
export async function withProductionMerchant(fn) {
  const dataDir = await mkdtemp(path.join(tmpdir(), "w829-mcp-iserror-"));
