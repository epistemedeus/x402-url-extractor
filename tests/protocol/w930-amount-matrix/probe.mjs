...
export async function listMcpTools(base) {
  ...
  const initialize = await fetch(`${base}/mcp`, {
    ...
    body: JSON.stringify({
      ...
      params: {
        ...
        capabilities: {},
        clientInfo: { name: "w930-amount-matrix", version: "0.0.1" },
      },
    }),
  });
  ...
  return {
    ...
    initializeBodyPreview: initializeText.slice(0, 200),
...
export async function startLocalMerchant() {
  const dataDir = await mkdtemp(join(tmpdir(), "w930-amount-matrix-"));
