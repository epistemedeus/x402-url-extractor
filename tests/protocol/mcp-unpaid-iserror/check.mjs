...
import { ROOT, loadJson, SEEDED_HTTP_200_AS_CHARGED } from "./paths.mjs";
...
function help() {
  return `x402 MCP unpaid tools/call is HTTP 200 isError, not settlement.

Usage:
  node tests/protocol/mcp-unpaid-iserror/check.mjs
  node tests/protocol/mcp-unpaid-iserror/check.mjs --cold
  node tests/protocol/mcp-unpaid-iserror/check.mjs --seeded-failure
  node tests/protocol/mcp-unpaid-iserror/check.mjs --all-fixtures
  node tests/protocol/mcp-unpaid-iserror/check.mjs <fixture.json>
...
async function main(argv = process.argv.slice(2)) {
  ...
  if (argv[0] === "--all-fixtures") {
    ...
    process.stderr.write(
      `mcp-unpaid-iserror fixtures: ${report.ok ? "pass" : "fail"} counted=${report.counted} passed=${report.passed}\n`,
    );
    return report.ok ? 0 : 1;
  }

  if (argv[0] === "--seeded-failure") {
    const report = runSeededFailure(SEEDED_HTTP_200_AS_CHARGED);
    print({
      ...report,
      fixture: "tests/protocol/mcp-unpaid-iserror/fixtures/reject/seeded-http-200-as-charged.json",
    });
    if (report.ok) {
      process.stderr.write("SEEDED_FAILURE must not pass unpaid HTTP 200 as charged/paid delivery\n");
      return 2;
    }
    process.stderr.write(
  ...
  if (argv.length === 0 || argv[0] === "--cold") {
    ...
    print(report);
    process.stderr.write(
      `mcp-unpaid-iserror cold: ${report.ok ? "pass" : "fail"} code=${report.code}`
        + ` list=${report.wire?.toolsListHttpStatus} call=${report.wire?.toolsCallHttpStatus}`
        + ` isError=${report.wire?.toolsCallIsError} paymentRequiredHeader=${report.wire?.paymentRequiredHeader}`
  ...
  if (fixture.expect === "reject") {
    if (report.ok) {
      process.stderr.write("reject fixture must not pass mcp-unpaid-iserror\n");
      return 2;
    }
    process.stderr.write(`rejected fixture id=${report.id} code=${report.code}\n`);
    return 1;
  }
  if (!report.ok && report.code === CODES.HTTP_200_CLASSIFIED_AS_CHARGED) {
