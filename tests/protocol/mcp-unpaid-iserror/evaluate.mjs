...
export function evaluateToolsCall(fixture = {}) {
  ...
  if (classified.kind !== "unpaid_mcp_challenge") {
    if (!violations.some((item) => item.code === CODES.PAYMENT_REQUIRED_HEADER
      || item.code === CODES.HTTP_402_NOT_MCP_ISERROR
      || item.code === CODES.HTTP_200_CLASSIFIED_AS_CHARGED
      || item.code === CODES.INVENTED_RECEIPT_FIELD)) {
      violations.push(violation(
        classified.hasPaymentRequiredHeader ? CODES.PAYMENT_REQUIRED_HEADER : CODES.MALFORMED_FIXTURE,
