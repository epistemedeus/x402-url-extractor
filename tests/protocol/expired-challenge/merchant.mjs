...
function extractMaxTimeout(body) {
  ...
  const n = Number(list[0]?.maxTimeoutSeconds);
  return Number.isSafeInteger(n) ? n : MAX_TIMEOUT_SECONDS;
}
...
async function startFakeFacilitator({ nowSec = () => Math.floor(Date.now() / 1000) } = {}) {
  ...
  const server = createHttpServer(async (req, res) => {
    ...
    if (req.method === "POST" && req.url === "/verify") {
      ...
      const window = classifyWindow({
        ...
        maxTimeoutSeconds: extractMaxTimeout(body),
      });
      const response = facilitatorVerifyFromWindow(window, extractPayer(auth));
    ...
    if (req.method === "POST" && req.url === "/settle") {
      ...
      if (window.status !== "within_window") {
        const response = {
          success: false,
          errorReason: window.invalidReason || "expired",
          transaction: "",
          network: NETWORK,
        };
...
export async function withMerchant(options, fn) {
  const dataDir = await mkdtemp(path.join(tmpdir(), "expired-challenge-"));
  const nowSec = typeof options?.nowSec === "function"
    ? options.nowSec
    : () => Math.floor(Date.now() / 1000);
  const facilitator = await startFakeFacilitator({ nowSec });
  ...
  const session = {
    ...
    async recordAttempt(phase, headers = {}, extra = {}) {
      ...
      return {
        ...
        maxTimeoutSeconds: Number.isSafeInteger(Number(accepted?.maxTimeoutSeconds))
          ? Number(accepted.maxTimeoutSeconds)
          : extra.maxTimeoutSeconds ?? null,
        validAfter: extra.validAfter ?? auth?.validAfter ?? null,
        validBefore: extra.validBefore ?? auth?.validBefore ?? null,
        observedAtSec,
    ...
    credential({
      ...
      const observedAtSec = nowSec();
      const after = validAfter ?? "0";
      const before = validBefore ?? String(observedAtSec + MAX_TIMEOUT_SECONDS);
...
export function traceBoundary(session) {
  return {
    ...
    source: "loopback server.js + expiry-aware fake facilitator",
  };
}
